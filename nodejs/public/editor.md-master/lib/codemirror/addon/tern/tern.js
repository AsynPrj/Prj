...
(function (mod) {
  ...
  function hint (ts, cm, c) {
    ts.request(cm, { type: 'completions', types: true, docs: true, urls: true }, function (error, data) {
      ...
      var completions = []; var after = ''
      var from = data.start; var to = data.end
      if (cm.getRange(Pos(from.line, from.ch - 2), from) == '["' &&
          cm.getRange(to, Pos(to.line, to.ch + 2)) != '"]') { after = '"]' }

      for (var i = 0; i < data.completions.length; ++i) {
        var completion = data.completions[i]; var className = typeToIcon(completion.type)
  ...
  function updateArgHints (ts, cm) {
    ...
    var ch; var argPos = lex.pos || 0; var tabSize = cm.getOption('tabSize')
    for (var line = cm.getCursor().line, e = Math.max(0, line - 9), found = false; line >= e; --line) {
      var str = cm.getLine(line); var extra = 0
      for (var pos = 0; ;) {
  ...
  function showArgHints (ts, cm, pos) {
    ...
    var cache = ts.cachedArgHints; var tp = cache.type
  ...
  function jumpToDef (ts, cm) {
    function inner (varName) {
      ...
      ts.server.request(buildRequest(ts, doc, req), function (error, data) {
        ...
        if (data.file) {
          var localDoc = ts.docs[data.file]; var found
  ...
  function jumpBack (ts, cm) {
    var pos = ts.jumpStack.pop(); var doc = pos && ts.docs[pos.file]
    if (!doc) return
  ...
  function findContext (doc, data) {
    ...
    var nearest; var nearestDist = Infinity
    while (cursor.findNext()) {
      var from = cursor.from(); var dist = Math.abs(from.line - start.line) * 10000
  ...
  function atInterestingExpression (cm) {
    var pos = cm.getCursor('end'); var tok = cm.getTokenAt(pos)
  ...
  function applyChanges (ts, changes) {
    ...
    for (var file in perFile) {
      var known = ts.docs[file]; var chs = perFile[file]
      if (!known) continue
  ...
  function buildRequest (ts, doc, query, pos) {
    var files = []; var offsetLines = 0; var allowFragments = !query.fullDocs
  ...
  function getFragmentAround (data, start, end) {
    var doc = data.doc
    var minIndent = null; var minLine = null; var endLine; var tabSize = 4
    for (var p = start.line - 1, min = Math.max(0, p - 50); p >= min; --p) {
      var line = doc.getLine(p); var fn = line.search(/\bfunction\b/)
      if (fn < 0) continue
  ...
  function tempTooltip (cm, content) {
    ...
    var mouseOnTip = false; var old = false

...
(function (mod) {
  ...
  function postUpdateDisplay (cm, update) {
    var force = update.force; var viewport = update.viewport
  ...
  function updateHeightsInViewport (cm) {
    ...
    for (var i = 0; i < display.view.length; i++) {
      var cur = display.view[i]; var height
  ...
  function applyTextInput (cm, inserted, deleted, sel) {
    ...
    var textLines = splitLines(inserted); var multiPaste = null
  ...
  TextareaInput.prototype = copyObj({
    init: function (display) {
      ...
      on(te, 'paste', function () {
        ...
        if (webkit && !cm.state.fakedLastChar && !(new Date() - cm.state.lastMiddleDown < 200)) {
          var start = te.selectionStart; var end = te.selectionEnd
          te.value += '$'
    ...
    reset: function (typing) {
      ...
      var minimal; var selected; var cm = this.cm; var doc = cm.doc
      if (cm.somethingSelected()) {
        this.prevInput = ''
      ...
      function rehide () {
        ...
        if (te.selectionStart != null) {
          ...
          var i = 0; var poll = function () {
            if (display.selForContextMenu == cm.doc.sel && te.selectionStart == 0) { operation(cm, commands.selectAll)(cm) } else if (i++ < 10) display.detectingSelectAll = setTimeout(poll, 500)
  ...
  ContentEditableInput.prototype = copyObj({
    ...
    showPrimarySelection: function () {
      var sel = window.getSelection(); var prim = this.cm.doc.sel.primary()
    ...
    pollSelection: function () {
      ...
      var sel = window.getSelection(); var cm = this.cm
  ...
  function posToDOM (cm, pos) {
    ...
    var order = getOrder(line); var side = 'left'
    if (order) {
  ...
  function nodeAndOffsetInLineMap (map, ch, bias) {
    ...
    for (var i = 0; i < map.length; i += 3) {
      var mStart = map[i]; var mEnd = map[i + 1]
      if (ch < mStart) {
        start = 0; end = 1
        collapse = 'left'
      } else if (ch < mEnd) {
  ...
  function estimateCoords (cm, pos) {
    var left = 0; var pos = clipPos(cm.doc, pos)
  ...
  function endOperation (cm) {
    var op = cm.curOp; var group = op.ownsGroup
    if (!group) return
  ...
  function leftButtonDown (cm, e, start) {
    ...
    var sel = cm.doc.sel; var modifier = mac ? e.metaKey : e.ctrlKey; var contained
    if (cm.options.dragDrop && dragAndDrop && !isReadOnly(cm) &&
        type == 'single' && (contained = sel.contains(start)) > -1 &&
        !sel.ranges[contained].empty()) { leftButtonStartDrag(cm, e, start, modifier) } else { leftButtonSelect(cm, e, start, type, modifier) }
  }
  ...
  function leftButtonStartDrag (cm, e, start, modifier) {
    ...
    var dragEnd = operation(cm, function (e2) {
      ...
      if (Math.abs(e.clientX - e2.clientX) + Math.abs(e.clientY - e2.clientY) < 10) {
        ...
        if (ie && ie_version == 9) { setTimeout(function () { document.body.focus(); display.input.focus() }, 20) }
      }
    })
  ...
  function leftButtonSelect (cm, e, start, type, addNew) {
    ...
    function extendTo (pos) {
      ...
      if (type == 'rect') {
        ...
        var anchor = oldRange.anchor; var head = pos
        if (type != 'single') {
  ...
  function ensureCursorVisible (cm) {
    ...
    var cur = cm.getCursor(); var from = cur; var to = cur
    if (!cm.options.lineWrapping) {
  ...
  CodeMirror.prototype = {
    ...
    moveV: methodOp(function (dir, unit) {
      var cm = this; var doc = this.doc; var goals = []
  ...
  function normalizeKeyName (name) {
    var parts = name.split(/-(?!$)/); var name = parts[parts.length - 1]
  ...
  var keyName = CodeMirror.keyName = function (event, noShift) {
    ...
    var base = keyNames[event.keyCode]; var name = base
  ...
  TextMarker.prototype.clear = function () {
    ...
    if (cm && this.collapsed && !cm.options.lineWrapping) {
      for (var i = 0; i < this.lines.length; ++i) {
        var visual = visualLine(this.lines[i]); var len = lineLength(visual)
  ...
  LineWidget.prototype.clear = function () {
    var cm = this.cm; var ws = this.line.widgets; var line = this.line; var no = lineNo(line)
  ...
  LineWidget.prototype.changed = function () {
    var oldH = this.height; var cm = this.cm; var line = this.line
    this.height = null
  ...
  function takeToken (cm, pos, precise, asArray) {
    ...
    pos = clipPos(doc, pos)
    var line = getLine(doc, pos.line); var state = getStateBefore(cm, pos.line, precise)
    var stream = new StringStream(line.text, cm.options.tabSize); var tokens
    if (asArray) tokens = []
  ...
  function buildLineContent (cm, lineView) {
    ...
    for (var i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
      var line = i ? lineView.rest[i - 1] : lineView.line; var order
      builder.pos = 0
  ...
  function buildToken (builder, text, style, startStyle, endStyle, title, css) {
    if (!text) return
    var special = builder.cm.options.specialChars; var mustWrap = false
  ...
  function lineEnd (cm, lineN) {
    var merged; var line = getLine(cm.doc, lineN)
    while (merged = collapsedSpanAtEnd(line)) {
      line = merged.find(1, true).line
      lineN = null
    }

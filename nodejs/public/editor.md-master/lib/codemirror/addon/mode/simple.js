...
(function (mod) {
  ...
  CodeMirror.simpleMode = function (config, states) {
    ...
    var states_ = {}; var meta = states.meta || {}; var hasIndentation = false
    for (var state in states) {
      if (state != meta && states.hasOwnProperty(state)) {
        var list = states_[state] = []; var orig = states[state]
    ...
    return function (stream, state) {
      ...
      if (state.local) {
        if (state.local.end && stream.match(state.local.end)) {
          ...
          var tok = state.local.mode.token(stream, state.localState); var m
  ...
  function indentFunction (states, meta) {
    return function (state, textAfter, line) {
      ...
      var pos = state.indent.length - 1; var rules = states[state.state]
      scan: for (;;) {

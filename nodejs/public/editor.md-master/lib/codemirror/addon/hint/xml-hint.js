...
(function (mod) {
  ...
  function getHints (cm, options) {
    ...
    var result = []; var replaceToken = false; var prefix
    var tag = /\btag\b/.test(token.type) && !/>$/.test(token.string)
    var tagName = tag && /^\w/.test(token.string); var tagStart
    ...
    if (!tag && !inner.state.tagName || tagType) {
      ...
      var cx = inner.state.context; var curTag = cx && tags[cx.tagName]
      ...
      var curTag = tags[inner.state.tagName]; var attrs = curTag && curTag.attrs
      ...
      if (token.type == 'string' || token.string == '=') { // A value
        ...
        var atName = before.match(/([^\s\u00a0=<>\"\']+)=$/); var atValues

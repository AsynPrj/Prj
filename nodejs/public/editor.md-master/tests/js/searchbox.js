...
(function (mod) {
  ...
  { define(['../../lib/codemirror'], mod) } else { mod(CodeMirror) }
})(function (CodeMirror) {
  'use strict'
  ...
  function SearchBox (cm) {
    ...
    function init () {
      ...
      el.addEventListener('click', function (e) {
        ...
        if (action && self[action]) { self[action]() } else if (self.commands[action]) { self.commands[action]() }
    ...
    function bindKeys () {
      ...
      var obj = {
        ...
        'Tab': function () {
          if (self.activeInput === self.replaceInput) { self.searchInput.focus() } else { self.replaceInput.focus() }
        }
      }
    ...
    function find (value, options, callback) {
      ...
      var noMatch; var searchCursor; var next; var prev; var matches; var cursor
      ...
      if (o.backwards) { position = o.skipCurrent ? 'from' : 'to' } else { position = o.skipCurrent ? 'to' : 'from' }
    ...
    this.replace = function () {
      if (!cm.getOption('readOnly')) { cm.replaceSelection(this.replaceInput.value, 'start') }
    }
    ...
    right = str.split('|').some(function (combination) {
      ...
      wrong = combination.split('-').some(function (key) {
        ...
        switch (key) {
          ...
          default:
            if (key.length === 1) { right = event.keyCode === key.charCodeAt(0) } else {

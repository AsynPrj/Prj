...
(function (mod) {
  ...
  function registerUpdate (dv) {
    ...
    var debounceChange; var updatingFast = false
  ...
  function syncScroll (dv, type) {
    ...
    var editor; var other; var now = +new Date()
    ...
    if (dv.mv.options.connect == 'align') {
      ...
      var halfScreen = 0.5 * sInfo.clientHeight; var midY = sInfo.top + halfScreen
  ...
  function markChanges (editor, diff, type, marks, from, to, classes) {
    var pos = Pos(0, 0)
    var top = Pos(from, 0); var bot = editor.clipPos(Pos(to - 1))
    var cls = type == DIFF_DELETE ? classes.del : classes.insert
    function markChunk (start, end) {
      var bfrom = Math.max(from, start); var bto = Math.min(to, end)
    ...
    for (var i = 0; i < diff.length; ++i) {
      var part = diff[i]; var tp = part[0]; var str = part[1]
      if (tp == DIFF_EQUAL) {
        ...
        if (tp == type) {
          ...
          var a = posMax(top, pos); var b = posMin(bot, end)
  ...
  function makeConnections (dv) {
    ...
    var vpEdit = dv.edit.getViewport(); var vpOrig = dv.orig.getViewport()
    var sTopEdit = dv.edit.getScrollInfo().top; var sTopOrig = dv.orig.getScrollInfo().top
  ...
  function getMatchingOrigLine (editLine, chunks) {
    var editStart = 0; var origStart = 0
  ...
  function alignChunks (dv, force) {
    ...
    var cm = [dv.orig, dv.edit]; var scroll = []
  ...
  function drawConnectorsForChunk (dv, chunk, sTopOrig, sTopEdit, w) {
    ...
    if (dv.copyButtons) {
      ...
      if (editOriginals) {
        ...
        copyReverse.chunk = { editFrom: chunk.origFrom,
          editTo: chunk.origTo,
  ...
  var MergeView = CodeMirror.MergeView = function (node, options) {
    ...
    var origLeft = options.origLeft; var origRight = options.origRight == null ? options.orig : options.origRight

    var hasLeft = origLeft != null; var hasRight = origRight != null
    var panes = 1 + (hasLeft ? 1 : 0) + (hasRight ? 1 : 0)
    var wrap = []; var left = this.left = null; var right = this.right = null
    var self = this
  ...
  function getChunks (diff) {
    var chunks = []
    var startEdit = 0; var startOrig = 0
    var edit = Pos(0, 0); var orig = Pos(0, 0)
    for (var i = 0; i < diff.length; ++i) {
      var part = diff[i]; var tp = part[0]
      if (tp == DIFF_EQUAL) {
        var startOff = startOfLineClean(diff, i) ? 0 : 1
        var cleanFromEdit = edit.line + startOff; var cleanFromOrig = orig.line + startOff
        moveOver(edit, part[1], null, orig)
        var endOff = endOfLineClean(diff, i) ? 1 : 0
        var cleanToEdit = edit.line + endOff; var cleanToOrig = orig.line + endOff
        if (cleanToEdit > cleanFromEdit) {
          if (i) {
            chunks.push({ origFrom: startOrig,
  ...
  function collapseIdenticalStretches (mv, margin) {
    ...
    var clear = []; var edit = mv.editor(); var off = edit.firstLine()
  ...
  function moveOver (pos, str, copy, other) {
    var out = copy ? Pos(pos.line, pos.ch) : pos; var at = 0
    for (;;) {
      var nl = str.indexOf('\n', at)
      if (nl == -1) break
      ++out.line

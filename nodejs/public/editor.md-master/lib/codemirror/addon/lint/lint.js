...
(function (mod) {
  ...
  function makeMarker (labels, severity, multiple, tooltips) {
    var marker = document.createElement('div'); var inner = marker
  ...
  function groupByLine (annotations) {
    ...
    for (var i = 0; i < annotations.length; ++i) {
      var ann = annotations[i]; var line = ann.from.line;
  ...
  function startLinting (cm) {
    var state = cm.state.lint; var options = state.options
  ...
  function onMouseOver (cm, e) {
    ...
    var box = target.getBoundingClientRect(); var x = (box.left + box.right) / 2; var y = (box.top + box.bottom) / 2
  ...
  CodeMirror.defineOption('lint', false, function (cm, val, old) {
    ...
    if (val) {
      var gutters = cm.getOption('gutters'); var hasLintGutter = false
